...
async def download(target_file):
    ...
    if input_str:
        ...
        if downloader.isSuccessful():
            await target_file.edit(f"Downloaded to `{downloaded_file_name}` successfully !!")
        else:
            ...
            if hasattr(media, "document"):
                ...
                if not filename:
                    if "audio" in mime_type:
                        filename = "audio_" + datetime.now().isoformat("_", "seconds") + ".ogg"
                    elif "video" in mime_type:
                        filename = "video_" + datetime.now().isoformat("_", "seconds") + ".mp4"
                ...
                start_time = datetime.now()
                result = await target_file.client.download_media(media, TEMP_DOWNLOAD_DIRECTORY)
            dl_time = (datetime.now() - start_time).seconds
        except Exception as e:  # pylint:disable=C0103,W0703
            await target_file.edit(str(e))
        else:
            try:
                await target_file.edit(f"Downloaded to `{result.name}` in `{dl_time}` seconds.")
            except AttributeError:
                await target_file.edit(f"Downloaded to `{result}` in `{dl_time}` seconds.")
    else:
...
async def upload(event):
    ...
    if os.path.exists(input_str):
        if os.path.isfile(input_str):
            ...
            up_time = (datetime.now() - start_time).seconds
            await event.respond(
                f"Uploaded `{len(lst_files)}` files in `{input_str}` folder " f"in `{up_time}` seconds."
            )
    else:
