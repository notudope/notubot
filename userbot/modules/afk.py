...
async def mention_afk(mention):
    ...
    if ISAFK and mention.message.mentioned:
        ...
        if day == 1:
            ...
            if day > 6:
                date = now + datetime.timedelta(days=-day, hours=-hours, minutes=-minutes)
        ...
        if mention.sender_id not in USERS or chat_title not in USERS:
            if AFKREASON:
                await mention.reply(f"**I'm not available right now.** (Since: {afk_str})" f"\nReason: `{AFKREASON}`.")
            else:
                await mention.reply(
            ...
            if USERS[mention.sender_id] % randint(2, 4) == 0:
                if AFKREASON:
                    await mention.reply(
                        f"**I'm not available right now.** (Since: {afk_str})" f"\nReason: `{AFKREASON}`."
                    )
                else:
                    await mention.reply(
                        f"**I'm not available right now.** (Since: {afk_str})" "\n**Please come back later.**"
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                ...
                if USERS[sender.sender_id] % randint(2, 4) == 0:
                    if AFKREASON:
                        await sender.reply(
                            "**I'm not available right now.** (Since: {afk_str})" f"\nReason: `{AFKREASON}`."
                        )
                    else:
                        await sender.reply(
        ...
        if BOTLOG:
            ...
            for i in USERS:
                if str(i).isnumeric():
                    ...
                    await notafk.client.send_message(
                        BOTLOG_CHATID,
                        "Anonymous admin in `" + i + "` sent you " + "`" + str(USERS[i]) + " message(s)`",
                    )
        COUNT_MSG = 0
        USERS = {}
        AFKREASON = None
