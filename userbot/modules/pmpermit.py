    ...
    if (
        ...
        if isinstance(chat, User):
            ...
            async for message in event.client.iter_messages(event.chat_id, reverse=True, limit=1):
                if message.text is not UNAPPROVED_MSG and message.sender_id == self_user.id:
                    try:
...
async def add_pmsg(cust_msg):
    ...
    if conf.lower() == "set":
        ...
        if BOTLOG:
            await cust_msg.client.send_message(BOTLOG_CHATID, f"***{status} Unapproved message :*** \n\n{msg}")
    ...
    if conf.lower() == "get":
        if custom_message is not None:
            await cust_msg.edit("***This is your current unapproved message:***" f"\n\n{custom_message}")
        else:
            await cust_msg.edit(
