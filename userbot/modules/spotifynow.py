        ...
        async with event.client.conversation(chat) as conv:
            ...
            if response.text.startswith("Ads."):
                ...
                if input_str == "s":
                    ...
                    await event.respond(f"[Play on Spotify]({link})", reply_to=spot, link_preview=False)
                elif not input_str:
                    downloaded_file_name = await event.client.download_media(response.media, TEMP_DOWNLOAD_DIRECTORY)
                    link = response.reply_markup.rows[0].buttons[0].url
