...
async def wiki(wiki_q):
    ...
    if BOTLOG:
        await wiki_q.client.send_message(BOTLOG_CHATID, f"Wiki query `{match}` was executed successfully")
...
async def urban_dict(ud_e):
    ...
    if int(meanlen) >= 0:
        ...
        if BOTLOG:
            await ud_e.client.send_message(BOTLOG_CHATID, "ud query `" + query + "` executed successfully.")
    else:
...
async def text_to_speech(query):
    ...
    if message:
        ...
        return await query.edit("`Give a text or reply to a message for Text-to-Speech!`")
    ...
    with open("k.mp3"):
        ...
        os.remove("k.mp3")
        if BOTLOG:
            await query.client.send_message(BOTLOG_CHATID, "Text to Speech executed successfully !")
        await query.delete()
...
async def lang(value):
    ...
    if util == "trt":
        ...
        if arg not in LANGUAGES:
            return await value.edit(f"`Invalid Language code !!`\n`Available language codes for TRT`:\n\n`{LANGUAGES}`")
    ...
    if BOTLOG:
        await value.client.send_message(BOTLOG_CHATID, f"`Language for {scraper} changed to {LANG.title()}.`")
...
async def download_video(v_url):
    ...
    if "audio" in dl_type:
        opts = {
            ...
            "outtmpl": os.path.join(TEMP_DOWNLOAD_DIRECTORY, str(s_time), "%(title)s.%(ext)s"),
            "quiet": True,
            "logtostderr": False,
        }
        audio = True
        ...
        quality = f"bestvideo[height<={reso}]+bestaudio/best[height<={reso}]" if reso else "bestvideo+bestaudio/best"
        opts = {
            "format": quality,
            "addmetadata": True,
            "key": "FFmpegMetadata",
    ...
    if audio:
        ...
        thumb_image = [
            x for x in glob(os.path.join(TEMP_DOWNLOAD_DIRECTORY, str(s_time), "*")) if not x.endswith(".mp3")
        ][0]
