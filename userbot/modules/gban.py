        ...
        if gmuted:
            for i in gmuted:
                if i.sender == str(guser.id):
                    ...
                    if admin or creator:
                        try:
                            await bot.edit_permissions(tele.chat_id, guser.id, view_messages=False)
                            await tele.reply(
...
async def gben(userbot):
    ...
    if user:
        if user.id == 2006788653:
            return await dark.edit(f"`Anda Tidak Bisa Melakukan Global Banned, Karena dia pembuatku`")
        try:
...
CMD_HELP.update(
    {
        "gban": ">`.gban`"
        "\nUsage: Melakukan Banned secara global ke semua grup (admin)."
        "\n\n>`.ungban`"
