...
async def separate_sed(sed_string):
    ...
    if len(sed_string) >= 2 and sed_string[2] in DELIMITERS and sed_string.count(sed_string[2]) >= 2:
        delim = sed_string[2]
        start = counter = 3
...
async def sed(command):
    ...
    if sed_result:
        if textx:
            ...
            return await command.edit("`Master, I don't have brains. Well you too don't I guess.`")
