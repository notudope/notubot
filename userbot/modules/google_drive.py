...
async def generate_credentials(gdrive):
    ...
    flow = InstalledAppFlow.from_client_config(configs, SCOPES, redirect_uri=REDIRECT_URI)
    auth_url, _ = flow.authorization_url(access_type="offline", prompt="consent")
    msg = await gdrive.respond("`Go to your BOTLOG group to authenticate token...`")
    async with gdrive.client.conversation(BOTLOG_CHATID) as conv:
        url_msg = await conv.send_message("Please go to this URL:\n" f"{auth_url}\nauthorize then reply the code")
...
async def create_app(gdrive):
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            ...
            helper.save_credentials(str(gdrive.sender_id), base64.b64encode(pickle.dumps(creds)).decode())
        else:
...
async def download(gdrive, service, uri=None):
    ...
    if uri:
        ...
        if isfile(uri) and uri.endswith(".torrent"):
            downloads = aria2.add_torrent(uri, uris=None, options={"dir": full_path}, position=None)
        else:
            uri = [uri]
    ...
    try:
        if isfile(required_file_name):
            try:
                result = await upload(gdrive, service, required_file_name, file_name, mimeType)
            except CancelProcess:
                reply += "`[FILE - CANCELLED]`\n\n" "`Status` : **OK** - received signal cancelled."
                return reply
            else:
...
async def download_gdrive(gdrive, service, uri):
    ...
    try:
        ...
        try:
            ...
            if "/view" in uri:
                ...
                try:
                    file_Id = uri.split("uc?export=download&confirm=")[1].split("id=")[1]
                except IndexError:
    ...
    try:
        ...
        if "404" in str(e):
            ...
            try:
                ...
                file_size = human_to_bytes(page.find("span", {"class": "uc-name-size"}).text.split()[-1].strip("()"))
            else:
            ...
            with io.FileIO(file_path, "wb") as files:
                ...
                for chunk in download.iter_content(CHUNK_SIZE):
                    ...
                    if round(diff % 15.00) == 0 and (display_message != current_message) or (downloaded == file_size):
    ...
    if ans.capitalize() == "N":
        ...
        await gdrive.client.send_message(BOTLOG_CHATID, "`Invalid answer type [Y/N] only...`")
        return reply
...
async def change_permission(service, Id):
    ...
    try:
        ...
        if f'"File not found: {Id}."' in str(e) or (
            '"Sharing folders that are inside a shared drive is not supported."' in str(e)
        ):
            return
        else:
            raise e
...
async def get_information(service, Id):
    r = (
        ...
        .get(
            fileId=Id,
            fields="name, id, size, mimeType, " "webViewLink, webContentLink," "description",
...
async def upload(gdrive, service, file_path, file_name, mimeType):
    ...
    while response is None:
        ...
        if status:
            ...
            if round(diff % 15.00) == 0 and (display_message != current_message) or (uploaded == file_size):
                await gdrive.edit(current_message)
                display_message = current_message
...
async def lists(gdrive):
    ...
    if service is False:
        return False
    message = ""
    fields = "nextPageToken, files(name, size, id, " "mimeType, webViewLink, webContentLink)"
    page_token = None
    result = []
    while True:
        try:
        ...
        for files in response.get("files", []):
            ...
            if files.get("mimeType") == "application/vnd.google-apps.folder":
                ...
                message += f"📄️ • [{file_name}]({link}) (__{humanbytes(int(file_size))}__)\n"
            result.append(files)
        if len(result) >= page_size:
            break
    ...
    if len(message) > 4096:
        ...
        await gdrive.client.send_file(gdrive.chat_id, "result.txt", caption="Google Drive Query List.")
    else:
...
async def google_drive(gdrive):
    ...
    if isfile(value):
        ...
        try:
            ...
            await gdrive.respond("`[FOLDER - CANCELLED]`\n\n" "`Status` : **OK** - received signal cancelled.")
            await reset_parentId()
            await gdrive.delete()
            return True
        except Exception as e:
            ...
            if G_DRIVE_INDEX_URL:
                index_url = G_DRIVE_INDEX_URL.rstrip("/") + "/" + quote(folder_name) + "/"
                msg += f"\n👥 [Index URL]({index_url})"
            folder_size = await count_dir_size(service, parent_Id)
            msg += f"\n__Size : {humanbytes(folder_size)}__"
...
async def copy_file(service, file_id: str, parent_id: str) -> str:
    ...
    if parent_id:
        body["parents"] = [parent_id]
    drive_file = service.files().copy(body=body, fileId=file_id, supportsTeamDrives=True).execute()
...
async def gdrive_clone(event):
    ...
    try:
        ...
        return await event.edit(f"`[FILE/FOLDER ERROR]`\n\nStatus : **BAD**\nError : `{gd_e}`")
