...
async def add_note(fltr):
    ...
    if msg and msg.media and not string:
        if BOTLOG_CHATID:
            ...
            return await fltr.edit("`Saving media as data for the note requires the BOTLOG_CHATID to be set.`")
    ...
    try:
        if not (await getnt.get_sender()).bot:
            ...
            if note and note.f_mesg_id:
                msg_o = await getnt.client.get_messages(entity=BOTLOG_CHATID, ids=int(note.f_mesg_id))
                ...
                await getnt.client.send_message(getnt.chat_id, note.reply, reply_to=message_id_to_reply)
    except AttributeError:
        pass
...
async def kick_marie_notes(kick):
    ...
    if BOTLOG:
        await kick.client.send_message(BOTLOG_CHATID, "I cleaned all Notes at " + str(kick.chat_id))
